...
$releaseApk = Join-Path $projectRoot 'releases\Kasir-Nusa-POS-1.4.0.apk'
$publicApk = Join-Path $projectRoot 'apps\web\downloads\Kasir-Nusa-POS-1.4.0.apk'
...
try {
    ...
    Write-Host ''
    Write-Host 'APK 1.4.0 dengan Firebase berhasil dibuat.' -ForegroundColor Green
    Write-Host "SHA-256: $hash"
    Write-Host 'Kembali ke Codex dan ketik: APK 1.4.0 BERHASIL'
} finally {
    $storePassword = $null
    $keyPassword = $null
