...
$releaseApk = Join-Path $projectRoot 'releases\Kasir-Nusa-POS-1.3.1.apk'
$publicApk = Join-Path $projectRoot 'apps\web\downloads\Kasir-Nusa-POS-1.3.1.apk'
...
try {
    ...
    Write-Host ''
    Write-Host 'APK 1.3.1 berhasil dibuat.' -ForegroundColor Green
    Write-Host "SHA-256: $hash"
    Write-Host 'Kembali ke Codex dan ketik: APK BERHASIL'
} finally {
    $storePassword = $null
    $keyPassword = $null
