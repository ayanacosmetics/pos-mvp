...
let promoCodeManuallyEdited=false;
let promoCodeSeed='';
const PROMO_NO_END='9999-12-31T15:59:59.999Z';

function generatedPromotionCode(name){
  const slug=String(name??'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toUpperCase().replace(/[^A-Z0-9]+/g,'-').replace(/^-|-$/g,'')||'PROMO';
  const suffix=promoCodeSeed||(promoCodeSeed=Date.now().toString(36).slice(-6).toUpperCase());
  return `${slug.slice(0,Math.max(1,29-suffix.length))}-${suffix}`;
}

function syncPromotionCodeFromName(){
  if(promoCodeManuallyEdited||el('promotion-form').dataset.editingPromoId)return;
  const name=el('promo-name').value.trim();
  el('promo-code').value=name?generatedPromotionCode(name):'';
}
...
function promoPayload() {
  ...
  let code=el('promo-code').value.trim().toUpperCase()||el('promotion-form').dataset.originalPromoCode||generatedPromotionCode(name);
  if(!el('promo-code').value.trim())el('promo-code').value=code;
  return {
...
function resetPromotionEditor(){
  const form=el('promotion-form');form.reset();form.dataset.editingPromoId='';form.dataset.originalPromoCode='';promoTargetProductIds.clear();promoEditorTab='identity';promoCodeManuallyEdited=false;promoCodeSeed='';
...
function editPromotion(versionId){
  ...
  el('promotion-form').dataset.editingPromoId=versionId;el('promotion-form').dataset.originalPromoCode=promo.code;promoCodeManuallyEdited=true;promoCodeSeed='';el('promo-code').value=promo.code;el('promo-name').value=promo.name;
...
async function publishPromotion(event) {
  ...
  try {
    ...
    toast(`${promo.code} versi ${promo.version} diterbitkan`);form.dataset.editingPromoId='';form.dataset.originalPromoCode='';
...
function renderRestockSourceSelector(){
  ...
  el('restock-source-guidance').innerHTML=poMode
    ? active
      ...
      : orders.length
        ? '<strong>Pilih PO terlebih dahulu.</strong><span>Sistem akan mengisi supplier, lokasi, daftar barang, dan modal dari PO. Jumlah datang tetap harus diperiksa.</span>'
  ...
  el('receive-all-po-items').classList.toggle('hidden',!active);
  if(active){
    el('restock-supplier').value=active.supplier_id;
    el('restock-location').value=active.location_id;
    const outstanding=active.items.reduce((sum,item)=>sum+Math.max(0,Number(item.remaining_qty??0)),0);
    const received=active.items.reduce((sum,item)=>sum+Number(item.received_qty??0),0);
    el('active-po-banner').innerHTML=`<div><span>${active.status==='PARTIALLY_RECEIVED'?'PENERIMAAN SISA PO':'PENERIMAAN BERDASARKAN PO'}</span><strong>${escapeHtml(active.po_no)} · ${escapeHtml(active.supplier_name)}</strong><small>${received>0?`${received.toLocaleString('id-ID')} pcs sudah masuk stok dan tidak dimuat ulang. `:''}Pemeriksaan ini hanya untuk sisa ${outstanding.toLocaleString('id-ID')} pcs.</small></div><button id="clear-active-po" class="button secondary" type="button">Ganti PO</button>`;
...
function restockStepIsValid(step, { notify = true } = {}) {
  ...
  if (step === 'items') {
    ...
    const invalid = rows.some((row) => {
      ...
      if (!rawQty || qty < 0 || !Number.isFinite(qty)) return true;
      if (qty === 0) return row.dataset.poLine !== 'true';
      if (!(cost >= 0) || !Number.isFinite(cost)) return true;
    ...
    if (!rows.some((row) => Number(row.querySelector('.restock-qty').value) > 0)) { if (notify) toast('Belum ada barang yang benar-benar diterima.'); return false; }
  }
  return true;
}
...
function renderRestockReview() {
  ...
  const inspection=buildRestockApprovalInspection(rows);
  const receiptSummary=state.activePurchaseOrder?`<div class="restock-review-receipt-summary"><span><small>Sisa PO sebelum diterima</small><strong>${Number(inspection.summary.orderedRemainingBaseQty).toLocaleString('id-ID')} pcs</strong></span><span><small>Akan masuk stok</small><strong>${Number(inspection.summary.actualReceivedBaseQty).toLocaleString('id-ID')} pcs</strong></span><span class="${inspection.summary.notReceivedBaseQty>0?'shortage':'complete'}"><small>Belum datang</small><strong>${Number(inspection.summary.notReceivedBaseQty).toLocaleString('id-ID')} pcs</strong></span></div>`:'';
  const items = rows.map((row) => {
    ...
    const expiry = row.querySelector('.restock-expiry').value.trim();
    if (qty === 0) return `<article class="restock-review-row restock-review-not-arrived"><div><strong>${escapeHtml(product?.name ?? row.dataset.product)}</strong><span class="badge neutral">TIDAK DATANG</span><small>Tidak menambah stok, batch, modal, atau pengajuan harga.</small></div><div><strong>0 ${escapeHtml(unit.name)}</strong></div></article>`;
  ...
  el('restock-review-list').innerHTML = heading + receiptSummary + items;
  updateRestockTotal();
}
...
function renderPurchaseOrders() {
  ...
  el('purchase-order-list').querySelectorAll('.po-resume-receipt').forEach((button)=>{
    button.addEventListener('click',(event)=>{event.stopPropagation();resumeOrderReceipt(button.dataset.id);});
...
function purchaseOrderDetailActions(order) {
  ...
  const receiveLabel=order.status==='PARTIALLY_RECEIVED'?`Terima sisa ${Number(order.outstanding_qty).toLocaleString('id-ID')} pcs`:'Terima barang';
  const resumeLabel=order.status==='PARTIALLY_RECEIVED'?'Kembali ke penerimaan sisa':'Lanjutkan pemeriksaan';
...
async function appendRestockLine(productId, qty = 1, newCost = 0, unit = 'pcs', options = {}) {
  ...
  if(options.poReceiving){
    row.dataset.poLine='true';
    row.dataset.poRemainingPurchaseQty=String(options.remainingPurchaseQty??'');
    row.dataset.poRemainingBaseQty=String(options.remainingBaseQty??'');
  }
  ...
  row.innerHTML = `
    ...
    <div class="restock-card-grid">
      <label>${options.poReceiving?'Diterima sekarang':'Jumlah'}<input class="restock-qty" type="number" min="${options.poReceiving?'0':'0.000001'}" step="any" value="${qty}" placeholder="${options.poReceiving?'Isi jumlah atau 0':''}">${options.poReceiving?`<small>Dipesan tersisa ${Number(options.remainingPurchaseQty).toLocaleString('id-ID')} ${escapeHtml(unit)}</small>`:''}</label>
  ...
  if(options.poReceiving)row.querySelector('.remove-restock')?.remove();
  ...
  row.querySelector('.restock-qty').addEventListener('input',()=>{
    updateRestockTotal();
    const rawQty=row.querySelector('.restock-qty').value.trim();
    if(options.poReceiving){
      if(rawQty===''||Number(rawQty)<=0)clearUnreceivedRestockState(row);
      else row.dataset.comparisonPending='true';
      updateRestockLineSummary(row);
    }
  });
  row.querySelector('.restock-qty').addEventListener('change',()=>updateRestockComparison(row));
  row.querySelector('.restock-unit').addEventListener('change',()=>changeRestockUnit(row));
  row.querySelector('.restock-expiry').addEventListener('input', formatExpiryInput);
...
function clearUnreceivedRestockState(row){
  row.dataset.comparisonPending='false';
  row.dataset.costNeedsApproval='false';
  row.dataset.quantityNeedsApproval='false';
  row.dataset.needsApproval='false';
  row.querySelector('.restock-price-proposal-slot')?.replaceChildren();
  const staffNote=row.querySelector('.restock-owner-approval-slot');
  if(staffNote){staffNote.classList.add('hidden');staffNote.replaceChildren();}
  const lastCost=row.querySelector('.last-cost'),lastMeta=row.querySelector('.last-meta'),costDelta=row.querySelector('.cost-delta');
  if(lastCost)lastCost.textContent='Menunggu jumlah';
  if(lastMeta)lastMeta.textContent='Modal diperiksa hanya untuk barang yang datang';
  if(costDelta)costDelta.innerHTML='<span class="badge neutral">BELUM DIPERIKSA</span>';
}

async function receiveAllPurchaseOrderItems(){
  const rows=[...document.querySelectorAll('.restock-line[data-po-line="true"]')];
  if(!state.activePurchaseOrder||!rows.length)return toast('Pilih PO terlebih dahulu.');
  rows.forEach((row)=>{
    const factor=Math.max(1,restockSelectedUnit(row).factor);
    row.querySelector('.restock-qty').value=Number(row.dataset.poRemainingBaseQty)/factor;
    updateRestockLineSummary(row);updateRestockTotal();
  });
...
function syncRestockApprovalRequirement(row){
  ...
  const rawQty=row.querySelector('.restock-qty')?.value?.trim()??'';
  if(row.dataset.poLine==='true'&&(rawQty===''||Number(rawQty)<=0)){clearUnreceivedRestockState(row);return;}
  const variance=restockPoQuantityVariance(row);
...
async function updateRestockComparison(row) {
  const rawQty=row.querySelector('.restock-qty')?.value?.trim()??'';
  if(row.dataset.poLine==='true'&&(rawQty===''||Number(rawQty)<=0)){clearUnreceivedRestockState(row);updateRestockLineSummary(row);return;}
  row.dataset.comparisonPending='true';
  try {
...
function buildRestockApprovalInspection(rows){
  const order=state.activePurchaseOrder;
  return buildPurchaseReceiptInspection({
    purchaseOrderId:order?.id??null,documentNo:el('restock-document').value,
    orderItems:order?.items??[],lines:rows.map((row)=>{
      const payload=restockRowPayload(row);
      const product=row.dataset.product
        ?state.products.find((item)=>item.id===row.dataset.product)
        :state.restockDraftProducts.get(row.dataset.productKey);
      return {...payload,productName:product?.name??'Produk',poLine:row.dataset.poLine==='true',
        verificationMethod:row.dataset.verificationMethod??''};
    })
  });
}
...
async function submitRestockForApproval(payload,rows,inspection){
  ...
  const result=await request('/api/restock-approvals',{method:'POST',body:JSON.stringify({...payload,items:rows.map((row)=>({...restockRowPayload(row),purchaseOrderId})),proposedPrices,inspection})});
...
function restockApprovalDetailMarkup(requestItem,canApprove){
  ...
  const inspection=requestItem.inspection?.summary;
  const inspectionBanner=inspection&&requestItem.sourcePurchaseOrderId?`<div class="restock-approval-inspection-summary"><span><small>Sisa PO saat diperiksa</small><strong>${Number(inspection.orderedRemainingBaseQty??0).toLocaleString('id-ID')} pcs</strong></span><span><small>Akan diterima</small><strong>${Number(inspection.actualReceivedBaseQty??0).toLocaleString('id-ID')} pcs</strong></span><span><small>Belum datang</small><strong>${Number(inspection.notReceivedBaseQty??0).toLocaleString('id-ID')} pcs</strong></span></div>`:'';
  ...
  return `<div class="restock-approval-detail-toolbar"><button class="button secondary back-restock-approvals" type="button">← Daftar pengajuan</button><div><span>Detail pengajuan</span><strong>${escapeHtml(requestItem.documentNo)}</strong></div></div>${revisionBanner}<article class="restock-approval-card status-${requestItem.status.toLowerCase()}" data-approval-id="${requestItem.id}"><header><div><p class="eyebrow">FAKTUR ${escapeHtml(requestItem.documentNo)}</p><h3>${escapeHtml(supplier)}</h3><small>${new Date(requestItem.requestedAt).toLocaleString('id-ID')} · ${requestItem.items.length} barang · ${money.format(totalCost)}</small></div><span class="badge ${status.tone} restock-approval-status">${status.label}</span></header>${inspectionBanner}${staffWaiting}<div class="restock-approval-items">${items}</div>${requestItem.decisionNote&&requestItem.status!=='REVISION_REQUIRED'?`<p class="restock-decision-note">Catatan: ${escapeHtml(requestItem.decisionNote)}</p>`:''}<footer>${pendingActions}${revisionActions}${requestItem.status==='APPROVED'?'<button class="button primary receive-approved-restock" type="button">Lanjut terima barang</button>':''}</footer></article>`;
}
...
async function resubmitRestockApproval(card,button){
  ...
  const revisedByKey=new Map(items.map((item)=>[item.productKey??item.productId,item]));
  const previousInspection=requestItem.inspection;
  const sourceOrder=state.purchaseOrders.find((order)=>order.id===requestItem.sourcePurchaseOrderId);
  const inspection=previousInspection?.lines?buildPurchaseReceiptInspection({
    purchaseOrderId:requestItem.sourcePurchaseOrderId,documentNo:requestItem.documentNo,
    orderItems:sourceOrder?.items??[],lines:previousInspection.lines.map((line)=>{
      const revised=revisedByKey.get(line.productKey??line.productId);
      return revised?{...line,baseQty:revised.baseQty,purchaseQty:revised.purchaseQty,
        purchaseUnitFactor:revised.purchaseUnitFactor??line.purchaseUnitFactor}:line;
    })
  }):{};
  button.disabled=true;button.textContent='Mengirim ulang…';
  try{await request(`/api/restock-approvals/${card.dataset.approvalId}/resubmit`,{method:'POST',body:JSON.stringify({items,inspection,note:card.querySelector('.revision-response-note')?.value??''})});toast('Perbaikan dikirim ulang kepada Owner.');state.activeRestockApprovalId=null;await loadRestockApprovals();}catch(error){toast(error.message);button.disabled=false;button.textContent='Kirim ulang ke Owner';}
}

async function receiveApprovedRestock(requestId,button){
  button.disabled=true;button.textContent='Menerima barang…';
  try{const receipt=await request(`/api/restock-approvals/${requestId}/receive`,{method:'POST',headers:{'idempotency-key':crypto.randomUUID()},body:'{}'});toast(receipt.po_status==='PARTIALLY_RECEIVED'?`Penerimaan ${receipt.document_no} berhasil · PO masih memiliki barang yang belum datang`:`Penerimaan ${receipt.document_no} berhasil · produk, harga, dan stok sudah aktif`);await refreshCatalog();if(state.session.permissions.includes('inventory.manage'))await loadInventory();await Promise.all([loadRestockApprovals(),loadPurchaseOrders()]);}catch(error){toast(error.message);button.disabled=false;button.textContent='Lanjut terima barang';}
}
...
async function receivePurchase() {
  ...
  let lines,rows,receivedRows;
  try {
    rows=[...document.querySelectorAll('.restock-line')];
    if(rows.some((row)=>row.dataset.poLine==='true'&&(!row.dataset.verificationMethod||row.querySelector('.restock-qty').value.trim()===''))){
      return showRestockReceiveError('Periksa semua barang PO: scan atau pilih manual, lalu isi jumlah yang datang atau 0 bila tidak datang.');
    }
    if(rows.some((row)=>Number(row.querySelector('.restock-qty').value)>0&&row.dataset.comparisonPending==='true')){
      return showRestockReceiveError('Tunggu pemeriksaan modal selesai, lalu coba kembali.');
    }
    receivedRows=rows.filter((row)=>Number(row.querySelector('.restock-qty').value)>0);
    lines=receivedRows.map(restockRowPayload);
  } catch (error) {
  ...
  if (!lines.length) return showRestockReceiveError('Belum ada barang yang benar-benar diterima. Isi jumlah aktual di atas 0.');
  if (lines.some((line) => !(line.baseQty > 0) || !(line.unitCost >= 0))) return showRestockReceiveError('Periksa jumlah dan modal setiap barang.');
  const inspection=buildRestockApprovalInspection(rows);
  const shortageMessage=state.activePurchaseOrder?purchaseReceiptShortageMessage(inspection.summary):'';
  if(shortageMessage&&!window.confirm(`${shortageMessage}\n\nBarang yang belum datang tidak akan masuk stok dan PO tetap berstatus Diterima sebagian.`))return;
  const payload = {
  ...
  button.disabled = true;
  const requiresApproval=receivedRows.some((row)=>row.dataset.needsApproval==='true');
  ...
  try {
    if(requiresApproval){
      await submitRestockForApproval(payload,receivedRows,inspection);
      return;
    }
...
function receiptPromotionLabel(promotion){
  return `Diskon: −${money.format(Math.abs(Number(promotion.discount??0)))}`;
}
...
function buildReceiptMarkup(receipt,payments=[],options={}){
  ...
  const paymentRows=layout.showPaymentDetail?payments.map((payment)=>`<div><span>${escapeHtml(receiptPaymentLabel(payment.method))}</span><strong>${money.format(payment.amount)}</strong></div>`).join(''):'';
  const cashSettlement=layout.showPaymentDetail&&paymentSummary.hasCash?`<div><span>Uang diterima</span><strong>${money.format(paymentSummary.cashReceived)}</strong></div><div><span>Kembalian</span><strong>${money.format(paymentSummary.change)}</strong></div>`:'';
  ...
  return `<section class="${classes}"><div class="receipt-head">${logo}${layout.showBusinessName?`<strong>${escapeHtml(business.name??'Kasir Nusa')}</strong>`:''}${layout.showOutletName?`<span>${escapeHtml(receipt.outletName??outlet.name??'Outlet')}</span>`:''}${layout.customHeader?`<small class="receipt-custom-header">${escapeHtml(layout.customHeader)}</small>`:''}${layout.showAddress&&address?`<small>${escapeHtml(address)}</small>`:''}${layout.showPhone&&phone?`<small>${escapeHtml(layout.contactLabel||'Tel.')} ${escapeHtml(phone)}</small>`:''}${layout.showDate?`<small>${new Date(receipt.occurredAt).toLocaleString('id-ID')}</small>`:''}${layout.showReceiptNumber?`<b>${escapeHtml(receipt.receiptNo)}</b>`:''}${receipt.status==='VOIDED'?'<b>VOID / DIBATALKAN</b>':''}${returnLabel?`<b class="receipt-return-status">${returnLabel}</b>`:''}</div><div class="receipt-meta">${meta}</div><div class="receipt-lines">${customerView.lines.map((line)=>`<div class="${Number(line.returnedQty)>0?'receipt-line-returned':''}"><span><strong>${escapeHtml(line.productName)}</strong>${layout.showPriceType&&priceLabel?`<small>${escapeHtml(priceLabel)}</small>`:''}<small>${line.qty} ${escapeHtml(line.unitName)} × ${money.format(line.customerUnitPrice)}</small>${(line.customerPromotions??[]).map((promotion)=>`<small class="receipt-line-promotion">${escapeHtml(receiptPromotionLabel(promotion))}</small>`).join('')}${Number(line.returnedQty)>0?`<small class="receipt-returned-note">Diretur ${Number(line.returnedQty).toLocaleString('id-ID')} ${escapeHtml(line.unitName)} · ${money.format(line.returnedTotal)}</small>`:''}</span><strong>${money.format(line.total)}</strong></div>`).join('')}</div><div class="receipt-totals"><div><span>Subtotal</span><strong>${money.format(customerView.subtotal)}</strong></div><div class="${returnTotal?'':'receipt-grand'}"><span>${returnTotal?'Total awal':'Total'}</span><strong>${money.format(customerView.grandTotal)}</strong></div>${Math.abs(receiptDiscount)>0.01?`<div class="receipt-savings"><span>Anda hemat</span><strong>${money.format(Math.abs(receiptDiscount))}</strong></div>`:''}${returnTotal?`<div class="receipt-return-deduction"><span>Retur / refund</span><strong>−${money.format(returnTotal)}</strong></div><div class="receipt-grand"><span>Total setelah retur</span><strong>${money.format(netTotal)}</strong></div>`:''}${paymentRows}${cashSettlement}</div>${returnTotal?`<div class="receipt-return-summary"><strong>${returnLabel}</strong>${(receipt.returns??[]).map((returned)=>`<small>${escapeHtml(returned.returnNo)} · ${new Date(returned.occurredAt).toLocaleString('id-ID')} · ${escapeHtml(returned.reason??'Retur')}</small>`).join('')}</div>`:''}${layout.showTransactionNote&&receipt.notes?`<p class="receipt-thanks"><strong>Catatan:</strong> ${escapeHtml(receipt.notes)}</p>`:''}${loyaltyBlock}${voucherBlock}${layout.customFooter?`<p class="receipt-thanks receipt-custom-footer">${escapeHtml(layout.customFooter)}</p>`:''}<p class="receipt-thanks">${escapeHtml(footer)}</p>${options.copyLabel?`<small class="receipt-copy-label">${escapeHtml(options.copyLabel)}</small>`:''}</section>`;
}
...
function historyReceiptShareText(sale){
  const customerView=customerReceiptView(sale.quote);
  const lines=customerView.lines.map((line)=>`${line.qty} ${line.unitName} ${line.productName}: ${money.format(line.total)}${(line.customerPromotions??[]).map((promotion)=>`\n  ${receiptPromotionLabel(promotion)}`).join('')}${Number(line.returnedQty)>0?`\n  Diretur ${Number(line.returnedQty).toLocaleString('id-ID')} ${line.unitName}: -${money.format(line.returnedTotal)}`:''}`).join('\n');
  ...
  const savings=Number(customerView.discountTotal)>0?`\nAnda hemat: ${money.format(customerView.discountTotal)}`:'';
  return `${state.business.name??'Kasir Nusa POS'}\nStruk ${sale.receiptNo}\n${new Date(sale.occurredAt).toLocaleString('id-ID')}\n\n${lines}\n\nTotal awal: ${money.format(sale.quote?.grandTotal??0)}${savings}${returnSummary}\nTerima kasih.`;
}
...
el('promo-name').addEventListener('input',syncPromotionCodeFromName);
el('promo-code').addEventListener('input',()=>{promoCodeManuallyEdited=Boolean(el('promo-code').value.trim());});
