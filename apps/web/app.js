...
function renderProductTable() {
  ...
  el('product-table').innerHTML=list.length?`<table class="product-admin-table"><thead><tr><th class="product-select-cell"><input id="select-all-products" type="checkbox" aria-label="Pilih semua barang yang tampil" ${allVisibleSelected?'checked':''}></th><th>SKU</th><th>Nama produk</th><th>Kategori</th><th>Merek</th><th>Varian</th><th>Satuan</th><th>Barcode</th><th>Harga umum</th><th>Stok</th><th>Min. stok</th><th>Status</th></tr></thead><tbody>${list.map((product)=>{
    ...
    const extraUnits=Math.max(0,product.units.length-1);
    const unitDetails=product.units.map((unit)=>`${unit.name} (isi ${unit.factor})`).join(' · ');
    ...
    return `<tr data-product-id="${product.id}" class="product-admin-row ${state.selectedProductIds.has(product.id)?'selected':''} ${actionsOpen?'actions-open':''}" tabindex="0" aria-expanded="${actionsOpen}" title="Tekan untuk menampilkan tindakan produk"><td class="product-select-cell"><input class="select-product" type="checkbox" aria-label="Pilih ${escapeHtml(product.name)}" ${state.selectedProductIds.has(product.id)?'checked':''}></td><td class="product-code" title="${escapeHtml(product.sku)}">${escapeHtml(product.sku)}</td><td class="product-name-cell" title="${escapeHtml(product.name)}"><strong>${escapeHtml(product.name)}</strong></td><td title="${escapeHtml(product.category)}">${escapeHtml(product.category||'-')}</td><td title="${escapeHtml(product.brand||'Tanpa merek')}">${escapeHtml(product.brand||'-')}</td><td title="${escapeHtml(variant||'Tanpa varian')}">${escapeHtml(variant||'-')}</td><td title="${escapeHtml(unitDetails)}">${escapeHtml(baseUnit.name||'-')}${extraUnits?` <small>+${extraUnits}</small>`:''}</td><td class="product-code" title="${escapeHtml(barcodeDetails||'Tanpa barcode')}">${escapeHtml(baseUnit.barcode||'-')}</td><td title="${escapeHtml(customPrices||money.format(prices.retail))}"><strong>${money.format(prices.retail)}</strong></td><td class="numeric-cell"><strong>${product.stockBase}</strong></td><td class="numeric-cell">${product.minimumStock||0}</td><td><span class="badge ${product.active?'ok':'danger'}">${product.active?'Aktif':'Nonaktif'}</span></td></tr>${actionsOpen?`<tr class="product-action-row" data-product-id="${product.id}"><td colspan="12"><div class="product-action-drawer"><small>${escapeHtml(product.name)} · ${escapeHtml(product.sku)}</small><button class="button secondary edit-product" type="button">Edit produk</button><button class="button ${product.active?'danger-button':'secondary'} toggle-product" type="button" data-active="${!product.active}">${product.active?'Nonaktifkan':'Aktifkan'}</button></div></td></tr>`:''}`;
...
function renderImportPreview(preview) {
  ...
  const keysByKind={
    ...
    CUSTOMERS:['code','name','phone','groupId'],SUPPLIERS:['code','name','phone','address']
  };
  const keys=keysByKind[preview.kind]??[];
  const labels = { sku:'No. barang / SKU',name:'Nama',baseUnit:'Satuan dasar',retailPrice:'Harga umum',openingQty:'Stok awal',minimumStock:'Stok minimum',trackExpiry:'Pantau EXP',unitName:'Nama satuan',factor:'Isi satuan dasar',barcode:'Barcode',variantGroup:'Kelompok varian',variantName:'Nama varian',customerGroup:'Tipe pelanggan',minQty:'Minimal beli',unitPrice:'Harga / satuan dasar',code:'Kode',phone:'Telepon',groupId:'Kelompok',address:'Alamat' };
...
async function loadImportHistory() {
  try {
    ...
    const labels = { PRODUCTS:'Produk',PRODUCT_UNITS:'Satuan barang',PRODUCT_VARIANTS:'Varian barang',PRODUCT_PRICES:'Harga pelanggan',CUSTOMERS:'Pelanggan',SUPPLIERS:'Supplier' };
