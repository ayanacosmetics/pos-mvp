...
test('laporan penjualan menyatukan ringkasan keuntungan dan riwayat struk', () => {
  ...
  assert.match(html, /data-report-view="purchases-history"[\s\S]*Laporan pembelian/);
  assert.doesNotMatch(html, /data-report-view="purchases"/);
...
test('laporan pembelian memakai data penerimaan tersimpan dan membuka dokumen asli', () => {
  ...
  assert.match(html, /id="purchase-report-metrics"/);
  assert.match(html, /id="purchase-report-search"/);
  assert.match(app, /function openPurchaseReportReceipt\(receipt\)/);
  assert.match(app, /function purchaseReportRange\(period=state\.purchaseReportPeriod\)/);
  assert.match(app, /function renderPurchaseReportReceipts\(\)/);
  assert.match(app, /STRUK PEMBELIAN/);
  assert.match(app, /data-purchase-report-id/);
  assert.match(html, /id="purchase-report-receipt-page"/);
  assert.match(html, /id="back-purchase-report"/);
  assert.doesNotMatch(html, /id="purchase-report-dialog"/);
  assert.match(css, /\.purchase-original-document/);
  assert.match(css, /\.purchase-receipt-page\{position:fixed;z-index:100;inset:0/);
  assert.match(css, /\.purchase-receipt-line\{display:grid/);
  assert.doesNotMatch(css, /\.purchase-original-document\{overflow-x:auto\}/);
