...
test('fitur gabungan dipisahkan menjadi tujuan halaman sendiri', async () => {
  ...
  for (const view of ['summary','performance','purchases-history','sales','sales-products','sales-categories','sales-addons','stock-flow','audit']) assert.match(html, new RegExp(`data-report-view="${view}"`));
  assert.doesNotMatch(html,/data-report-view="purchases"/);
  assert.doesNotMatch(html,/data-report-view="sales-history"/);
  assert.match(html,/id="sales-report-subnav-toggle"/);
  assert.match(html,/id="sales-report-subnav"/);
