...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.8\.1-cloud/);
  assert.match(html, /Laporan Terpisah · v2\.8\.1/);
  assert.match(worker, /nusa-pos-shell-v86/);
  assert.equal(JSON.parse(pkgText).version, '2.8.1');
});
