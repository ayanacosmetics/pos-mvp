...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /1\.20\.1-cloud/);
  assert.match(html, /Paket Operasional · v1\.20/);
  assert.match(worker, /nusa-pos-shell-v38/);
  assert.equal(JSON.parse(pkgText).version, '1.20.1');
});
