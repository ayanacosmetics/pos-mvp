...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.16\.39-cloud/);
  assert.match(html, /Tombol Kasir ringkas .* v2\.16\.39/);
  assert.match(worker, /nusa-pos-shell-v150/);
  assert.equal(JSON.parse(pkgText).version, '2.16.39');
});
