...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.16\.33-cloud/);
  assert.match(html, /Poin impor per struk .* v2\.16\.33/);
  assert.match(worker, /nusa-pos-shell-v144/);
  assert.equal(JSON.parse(pkgText).version, '2.16.33');
});
