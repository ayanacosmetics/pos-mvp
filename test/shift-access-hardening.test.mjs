...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.2\.2-cloud/);
  assert.match(html, /Kas Tunai Cepat · v2\.2\.2/);
  assert.match(worker, /nusa-pos-shell-v58/);
  assert.equal(JSON.parse(pkgText).version, '2.2.2');
});
