...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /1\.23\.3-cloud/);
  assert.match(html, /Paket Operasional · v1\.23\.3/);
  assert.match(worker, /nusa-pos-shell-v48/);
  assert.equal(JSON.parse(pkgText).version, '1.23.3');
});
