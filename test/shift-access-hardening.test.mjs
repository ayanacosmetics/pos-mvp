...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.5\.0-cloud/);
  assert.match(html, /Tipe Pelanggan & Harga · v2\.5\.0/);
  assert.match(worker, /nusa-pos-shell-v76/);
  assert.equal(JSON.parse(pkgText).version, '2.5.0');
});
