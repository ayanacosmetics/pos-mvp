...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.9\.7-cloud/);
  assert.match(html, /Filter Penjualan · v2\.9\.7/);
  assert.match(worker, /nusa-pos-shell-v96/);
  assert.equal(JSON.parse(pkgText).version, '2.9.7');
});
