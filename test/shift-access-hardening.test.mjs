...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.4\.9-cloud/);
  assert.match(html, /Hak Akses Akun · v2\.4\.9/);
  assert.match(worker, /nusa-pos-shell-v73/);
  assert.equal(JSON.parse(pkgText).version, '2.4.9');
});
