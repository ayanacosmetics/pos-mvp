...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /1\.24\.0-cloud/);
  assert.match(html, /Paket Operasional · v1\.24\.0/);
  assert.match(worker, /nusa-pos-shell-v49/);
  assert.equal(JSON.parse(pkgText).version, '1.24.0');
});
