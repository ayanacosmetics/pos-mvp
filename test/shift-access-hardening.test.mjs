...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.16\.1-cloud/);
  assert.match(html, /Label Tajam & Sesuai Stok .* v2\.16\.1/);
  assert.match(worker, /nusa-pos-shell-v112/);
  assert.equal(JSON.parse(pkgText).version, '2.16.1');
});
