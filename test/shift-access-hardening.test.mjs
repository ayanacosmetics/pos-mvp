...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.16\.12-cloud/);
  assert.match(html, /Manajemen Stok .* v2\.16\.12/);
  assert.match(worker, /nusa-pos-shell-v123/);
  assert.equal(JSON.parse(pkgText).version, '2.16.12');
});
