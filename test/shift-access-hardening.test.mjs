...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.16\.18-cloud/);
  assert.match(html, /Reset & Pulihkan .* v2\.16\.18/);
  assert.match(worker, /nusa-pos-shell-v129/);
  assert.equal(JSON.parse(pkgText).version, '2.16.18');
});
