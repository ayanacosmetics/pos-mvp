...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.16\.11-cloud/);
  assert.match(html, /Jarak Label Presisi .* v2\.16\.11/);
  assert.match(worker, /nusa-pos-shell-v122/);
  assert.equal(JSON.parse(pkgText).version, '2.16.11');
});
