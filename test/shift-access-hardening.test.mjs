...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /1\.27\.0-cloud/);
  assert.match(html, /Paket Multi-outlet · v1\.27\.0/);
  assert.match(worker, /nusa-pos-shell-v53/);
  assert.equal(JSON.parse(pkgText).version, '1.27.0');
});
