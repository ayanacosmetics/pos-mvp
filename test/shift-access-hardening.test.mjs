...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.16\.41-cloud/);
  assert.match(html, /Restok barang baru .* v2\.16\.41/);
  assert.match(worker, /nusa-pos-shell-v152/);
  assert.equal(JSON.parse(pkgText).version, '2.16.41');
});
