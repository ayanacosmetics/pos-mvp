...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.16\.38-cloud/);
  assert.match(html, /Kasir mobile tetap .* v2\.16\.38/);
  assert.match(worker, /nusa-pos-shell-v149/);
  assert.equal(JSON.parse(pkgText).version, '2.16.38');
});
