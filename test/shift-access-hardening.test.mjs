...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.16\.28-cloud/);
  assert.match(html, /Migrasi pelanggan Kaspin .* v2\.16\.28/);
  assert.match(worker, /nusa-pos-shell-v139/);
  assert.equal(JSON.parse(pkgText).version, '2.16.28');
});
