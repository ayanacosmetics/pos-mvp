...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.13\.0-cloud/);
  assert.match(html, /Excel Data Massal · v2\.13\.0/);
  assert.match(worker, /nusa-pos-shell-v104/);
  assert.equal(JSON.parse(pkgText).version, '2.13.0');
});
