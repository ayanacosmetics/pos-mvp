...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /1\.21\.1-cloud/);
  assert.match(html, /Paket Operasional · v1\.21/);
  assert.match(worker, /nusa-pos-shell-v40/);
  assert.equal(JSON.parse(pkgText).version, '1.21.1');
});
