...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.16\.4-cloud/);
  assert.match(html, /Tabel Produk Ringkas .* v2\.16\.4/);
  assert.match(worker, /nusa-pos-shell-v115/);
  assert.equal(JSON.parse(pkgText).version, '2.16.4');
});
