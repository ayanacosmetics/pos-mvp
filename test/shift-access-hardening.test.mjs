...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.9\.3-cloud/);
  assert.match(html, /Struk Retur Aktual · v2\.9\.3/);
  assert.match(worker, /nusa-pos-shell-v92/);
  assert.equal(JSON.parse(pkgText).version, '2.9.3');
});
