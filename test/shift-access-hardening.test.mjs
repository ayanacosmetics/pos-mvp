...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.16\.3-cloud/);
  assert.match(html, /Cetak Label Staff .* v2\.16\.3/);
  assert.match(worker, /nusa-pos-shell-v114/);
  assert.equal(JSON.parse(pkgText).version, '2.16.3');
});
