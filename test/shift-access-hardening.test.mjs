...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.13\.1-cloud/);
  assert.match(html, /Sesi Permanen · v2\.13\.1/);
  assert.match(worker, /nusa-pos-shell-v105/);
  assert.equal(JSON.parse(pkgText).version, '2.13.1');
});
