...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.16\.9-cloud/);
  assert.match(html, /Kalibrasi Label Presisi .* v2\.16\.9/);
  assert.match(worker, /nusa-pos-shell-v120/);
  assert.equal(JSON.parse(pkgText).version, '2.16.9');
});
