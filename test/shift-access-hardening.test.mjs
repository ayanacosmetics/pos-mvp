...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.16\.23-cloud/);
  assert.match(html, /Performa katalog .* v2\.16\.23/);
  assert.match(worker, /nusa-pos-shell-v134/);
  assert.equal(JSON.parse(pkgText).version, '2.16.23');
});
