...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.4\.4-cloud/);
  assert.match(html, /Identitas Final · v2\.4\.4/);
  assert.match(worker, /nusa-pos-shell-v68/);
  assert.equal(JSON.parse(pkgText).version, '2.4.4');
});
