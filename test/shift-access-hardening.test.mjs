...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.16\.22-cloud/);
  assert.match(html, /Migrasi Kaspin .* v2\.16\.22/);
  assert.match(worker, /nusa-pos-shell-v133/);
  assert.equal(JSON.parse(pkgText).version, '2.16.22');
});
