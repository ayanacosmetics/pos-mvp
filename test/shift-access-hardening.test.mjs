...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.8\.2-cloud/);
  assert.match(html, /Pratinjau Struk · v2\.8\.2/);
  assert.match(worker, /nusa-pos-shell-v87/);
  assert.equal(JSON.parse(pkgText).version, '2.8.2');
});
