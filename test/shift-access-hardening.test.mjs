...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.1\.0-cloud/);
  assert.match(html, /Akuntansi Inti · v2\.1\.0/);
  assert.match(worker, /nusa-pos-shell-v55/);
  assert.equal(JSON.parse(pkgText).version, '2.1.0');
});
