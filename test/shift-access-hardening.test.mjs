...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.16\.35-cloud/);
  assert.match(html, /Tabel pelanggan tetap .* v2\.16\.35/);
  assert.match(worker, /nusa-pos-shell-v146/);
  assert.equal(JSON.parse(pkgText).version, '2.16.35');
});
