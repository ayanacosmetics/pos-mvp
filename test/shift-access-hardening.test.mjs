...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.16\.15-cloud/);
  assert.match(html, /Reset Data Aman .* v2\.16\.15/);
  assert.match(worker, /nusa-pos-shell-v126/);
  assert.equal(JSON.parse(pkgText).version, '2.16.15');
});
