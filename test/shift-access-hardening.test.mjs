...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.15\.1-cloud/);
  assert.match(html, /Barcode Label Ringkas .* v2\.15\.1/);
  assert.match(worker, /nusa-pos-shell-v110/);
  assert.equal(JSON.parse(pkgText).version, '2.15.1');
});
