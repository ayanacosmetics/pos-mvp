...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.16\.34-cloud/);
  assert.match(html, /Direktori pelanggan .* v2\.16\.34/);
  assert.match(worker, /nusa-pos-shell-v145/);
  assert.equal(JSON.parse(pkgText).version, '2.16.34');
});
