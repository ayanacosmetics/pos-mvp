...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.0\.0-cloud/);
  assert.match(html, /Pilot & Hardening · v2\.0\.0/);
  assert.match(worker, /nusa-pos-shell-v54/);
  assert.equal(JSON.parse(pkgText).version, '2.0.0');
});
