...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.16\.29-cloud/);
  assert.match(html, /Log aktivitas staff .* v2\.16\.29/);
  assert.match(worker, /nusa-pos-shell-v140/);
  assert.equal(JSON.parse(pkgText).version, '2.16.29');
});
