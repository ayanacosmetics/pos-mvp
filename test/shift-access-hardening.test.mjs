...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.3\.4-cloud/);
  assert.match(html, /Pilih Langsung · v2\.3\.4/);
  assert.match(worker, /nusa-pos-shell-v63/);
  assert.equal(JSON.parse(pkgText).version, '2.3.4');
});
