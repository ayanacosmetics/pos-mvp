...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.16\.0-cloud/);
  assert.match(html, /Editor Label Fleksibel .* v2\.16\.0/);
  assert.match(worker, /nusa-pos-shell-v111/);
  assert.equal(JSON.parse(pkgText).version, '2.16.0');
});
