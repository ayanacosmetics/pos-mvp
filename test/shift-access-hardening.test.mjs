...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.16\.27-cloud/);
  assert.match(html, /Migrasi struk Kaspin .* v2\.16\.27/);
  assert.match(worker, /nusa-pos-shell-v138/);
  assert.equal(JSON.parse(pkgText).version, '2.16.27');
});
