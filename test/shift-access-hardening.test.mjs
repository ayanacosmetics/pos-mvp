...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.16\.45-cloud/);
  assert.match(html, /Pendaftaran Owner pulih .* v2\.16\.45/);
  assert.match(worker, /nusa-pos-shell-v156/);
  assert.equal(JSON.parse(pkgText).version, '2.16.45');
});
