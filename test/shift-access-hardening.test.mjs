...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.10\.0-cloud/);
  assert.match(html, /Laporan Transaksi · v2\.10\.0/);
  assert.match(worker, /nusa-pos-shell-v97/);
  assert.equal(JSON.parse(pkgText).version, '2.10.0');
});
