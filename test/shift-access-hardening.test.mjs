...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.10\.2-cloud/);
  assert.match(html, /Jurnal Arus Stok · v2\.10\.2/);
  assert.match(worker, /nusa-pos-shell-v99/);
  assert.equal(JSON.parse(pkgText).version, '2.10.2');
});
