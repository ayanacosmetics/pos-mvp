...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.16\.44-cloud/);
  assert.match(html, /Kasir lebih ringan .* v2\.16\.44/);
  assert.match(worker, /nusa-pos-shell-v155/);
  assert.equal(JSON.parse(pkgText).version, '2.16.44');
});
