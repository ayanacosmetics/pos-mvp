...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.7\.0-cloud/);
  assert.match(html, /Voucher QR Struk · v2\.7\.0/);
  assert.match(worker, /nusa-pos-shell-v81/);
  assert.equal(JSON.parse(pkgText).version, '2.7.0');
});
