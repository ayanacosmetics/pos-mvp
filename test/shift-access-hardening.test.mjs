...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.9\.4-cloud/);
  assert.match(html, /Laporan Penjualan Terpadu · v2\.9\.4/);
  assert.match(worker, /nusa-pos-shell-v93/);
  assert.equal(JSON.parse(pkgText).version, '2.9.4');
});
