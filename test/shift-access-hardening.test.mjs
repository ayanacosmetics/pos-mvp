...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.9\.1-cloud/);
  assert.match(html, /Struk Layar Penuh · v2\.9\.1/);
  assert.match(worker, /nusa-pos-shell-v90/);
  assert.equal(JSON.parse(pkgText).version, '2.9.1');
});
