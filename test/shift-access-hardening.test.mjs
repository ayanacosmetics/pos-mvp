...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.16\.10-cloud/);
  assert.match(html, /Produk Lebih Ringkas .* v2\.16\.10/);
  assert.match(worker, /nusa-pos-shell-v121/);
  assert.equal(JSON.parse(pkgText).version, '2.16.10');
});
