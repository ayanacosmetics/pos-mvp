...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.16\.53-cloud/);
  assert.match(html, /Kamera absensi terpandu .* v2\.16\.53/);
  assert.match(worker, /nusa-pos-shell-v164/);
  assert.equal(JSON.parse(pkgText).version, '2.16.53');
});
