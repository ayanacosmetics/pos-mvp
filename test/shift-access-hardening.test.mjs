...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.16\.2-cloud/);
  assert.match(html, /Barcode Batang-Ruang .* v2\.16\.2/);
  assert.match(worker, /nusa-pos-shell-v113/);
  assert.equal(JSON.parse(pkgText).version, '2.16.2');
});
