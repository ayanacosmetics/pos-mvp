...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.16\.40-cloud/);
  assert.match(html, /Editor keranjang ringkas .* v2\.16\.40/);
  assert.match(worker, /nusa-pos-shell-v151/);
  assert.equal(JSON.parse(pkgText).version, '2.16.40');
});
