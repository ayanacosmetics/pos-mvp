...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.10\.1-cloud/);
  assert.match(html, /Laporan Kategori &amp; Stok · v2\.10\.1/);
  assert.match(worker, /nusa-pos-shell-v98/);
  assert.equal(JSON.parse(pkgText).version, '2.10.1');
});
