...
test('versi kandidat final tampil pada API, aplikasi, dan cache PWA', async () => {
  ...
  assert.match(api, /2\.8\.3-cloud/);
  assert.match(html, /Laporan Ringkas · v2\.8\.3/);
  assert.match(worker, /nusa-pos-shell-v88/);
  assert.equal(JSON.parse(pkgText).version, '2.8.3');
});
