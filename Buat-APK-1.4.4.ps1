...
$releaseApk = Join-Path $projectRoot 'releases\Kasir-Nusa-POS-1.4.4.apk'
$publicApk = Join-Path $projectRoot 'apps\web\downloads\Kasir-Nusa-POS-1.4.4.apk'

if (-not (Test-Path -LiteralPath $keystore)) { throw "Kunci permanen tidak ditemukan: $keystore" }
if (-not (Test-Path -LiteralPath (Join-Path $androidRoot 'app\google-services.json'))) { throw 'Konfigurasi Firebase Android belum terpasang.' }
...
try {
    ...
    if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $builtApk)) { throw 'Build APK gagal. Periksa password dan alias key.' }
    ...
    Write-Host ''
    Write-Host 'APK 1.4.4 dengan pencegahan restart scanner berhasil dibuat.' -ForegroundColor Green
    Write-Host "SHA-256: $hash"
    Write-Host 'Kembali ke Codex dan ketik: APK 1.4.4 BERHASIL'
} finally {
    $storePassword = $null
    $keyPassword = $null
