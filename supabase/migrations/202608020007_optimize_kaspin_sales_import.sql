-- Avoid repeatedly scanning the complete JSON package for every Kaspin receipt.
begin;
...
begin
  ...
  for v_receipt in
    select
      ...
      max(nullif(value->>'profit','')::numeric) source_profit,
      ...
      bool_or(coalesce((value->>'receiptOnly')::boolean,false) and upper(coalesce(value->>'status',''))='VOIDED') is_voided,
      min(nullif(trim(value->>'voidReason'),'')) filter(where coalesce((value->>'receiptOnly')::boolean,false)) void_reason,
      coalesce((min(value->>'legacyLines') filter(where coalesce((value->>'receiptOnly')::boolean,false)))::jsonb,'[]'::jsonb) legacy_lines,
      coalesce(jsonb_agg(value) filter(
        where not coalesce((value->>'receiptOnly')::boolean,false)
          and nullif(value->>'productId','') is not null
      ),'[]'::jsonb) item_lines,
      sum(case when coalesce((value->>'receiptOnly')::boolean,false) then 0 else (value->>'lineGross')::numeric end) item_subtotal,
      sum(case when coalesce((value->>'receiptOnly')::boolean,false) then 0 else (value->>'quantity')::numeric*(value->>'unitCost')::numeric end) mapped_cost
    from jsonb_array_elements(p_rows)
    where nullif(trim(value->>'transactionCode'),'') is not null
  ...
  loop
    ...
    v_grand_total:=greatest(coalesce(v_receipt.grand_total,0),0);
    v_subtotal:=greatest(coalesce(v_receipt.item_subtotal,0),v_grand_total);
    v_discount_total:=greatest(v_subtotal-v_grand_total,0);
    v_source_profit:=v_receipt.source_profit;
    v_cost_total:=case when v_source_profit is not null
      then greatest(v_grand_total-v_source_profit,0)
      else greatest(coalesce(v_receipt.mapped_cost,0),0) end;
    ...
    v_status:=case when v_receipt.is_voided then 'VOIDED' else 'COMPLETED' end;
    v_void_reason:=case when v_status='VOIDED' then coalesce(v_receipt.void_reason,'Retur atau pembatalan dari riwayat Kasir Pintar') else null end;
    v_legacy_lines:=coalesce(v_receipt.legacy_lines,'[]'::jsonb);
    ...
    insert into public.sales(
      ...
      v_status,v_occurred_at,coalesce(v_receipt.note,'Impor riwayat Kasir Pintar'),
      'KASPIN',v_transaction_code,v_receipt.source_cashier,
      jsonb_build_object(
        'customerName',v_receipt.customer_name,'customerEmail',v_receipt.customer_email,
        'sourceProfit',v_source_profit,'legacyLines',v_legacy_lines,
        'legacyStatus',case when v_status='VOIDED' then 'RETURN_OR_VOID' else 'COMPLETED' end
      ),v_void_reason,case when v_status='VOIDED' then v_occurred_at else null end,
      case when v_status='VOIDED' then p_actor_id else null end
    ) returning id into v_sale_id;

    for v_line in select value from jsonb_array_elements(v_receipt.item_lines)
    loop
    ...
    if v_status='VOIDED' then v_voided:=v_voided+1; else v_completed:=v_completed+1; end if;
  end loop;
  ...
  insert into public.import_jobs(
    ...
    jsonb_build_object('kind','KASPIN_SALES','total',jsonb_array_length(p_rows),
      'created',v_receipts,'updated',0,'receipts',v_receipts,'completed',v_completed,
      'voided',v_voided,'items',v_items,'stockQuantityChanged',false)
  ...
  values(
    ...
    jsonb_build_object('fileName',p_file_name,'receipts',v_receipts,'completed',v_completed,
      'voided',v_voided,'items',v_items,'stockQuantityChanged',false)
  );
