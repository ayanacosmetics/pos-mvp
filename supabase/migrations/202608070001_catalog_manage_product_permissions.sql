-- Align database product mutations with the granular catalog.manage permission
-- already enforced by the API. Destructive bulk deletion remains Owner/Admin only.
  ...
  select exists(
    ...
    where tenant_id=p_tenant_id
      ...
      and (
        role in ('OWNER','ADMIN')
        or case
          when custom_permissions is null then role='MANAGER'
          else 'catalog.manage'=any(custom_permissions)
        end
      )
  );
$$;
...
begin
  if not public.can_manage_product_catalog_v1(p_tenant_id,p_actor_id) then
    raise exception 'Akun tidak memiliki izin Produk & harga';
  end if;
