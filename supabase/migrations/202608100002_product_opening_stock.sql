-- Create a catalog product and its optional opening stock in one transaction.
-- Opening cost is deliberately restricted to actors who may manage inventory
-- and view purchasing cost; edits to existing products never alter stock.
...
begin
  ...
  if v_quantity=0 then
    return public.save_product_v6(p_tenant_id,p_actor_id,p_product);
  end if;
  ...
  if not public.profile_has_permission_v1(p_tenant_id,p_actor_id,'catalog.manage')
    or not public.profile_has_permission_v1(p_tenant_id,p_actor_id,'inventory.manage')
    or not public.profile_has_permission_v1(p_tenant_id,p_actor_id,'purchasing.view_cost') then
    raise exception 'Akun tidak memiliki izin stok dan modal awal';
  end if;
