-- Kasir Nusa POS - preserve Kasir Pintar multi-unit selling prices during extension import
begin;
...
begin
  ...
  for v_row in select value from jsonb_array_elements(p_rows) loop
    ...
    if p_kind='PRODUCT_UNITS' then
      ...
      v_price:=nullif(v_row->>'unitPriceTotal','')::numeric;
      if v_name='' or v_factor<=0 then raise exception 'Nama atau isi satuan SKU % tidak valid',v_sku;end if;
      if v_price is not null and v_price<=0 then raise exception 'Harga satuan SKU % tidak valid',v_sku;end if;
      ...
      if v_price is not null then
        ...
        where tenant_id=p_tenant_id and product_id=v_product and customer_group_id='retail'
          and min_base_qty=v_factor and starts_at is null and ends_at is null;
        insert into public.price_rules(
          tenant_id,product_id,customer_group_id,min_base_qty,unit_price_base,priority,source,policy_tenant_id
        ) values(p_tenant_id,v_product,'retail',v_factor,v_price/v_factor,60,'MANUAL',null);
      end if;
