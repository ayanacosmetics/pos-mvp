...
$releaseApk = Join-Path $projectRoot 'releases\Kasir-Nusa-POS-1.4.1.apk'
$publicApk = Join-Path $projectRoot 'apps\web\downloads\Kasir-Nusa-POS-1.4.1.apk'
...
try {
    ...
    Write-Host ''
    Write-Host 'APK 1.4.1 dengan izin lokasi berhasil dibuat.' -ForegroundColor Green
    Write-Host "SHA-256: $hash"
    Write-Host 'Kembali ke Codex dan ketik: APK 1.4.1 BERHASIL beserta SHA-256 di atas'
} finally {
    $storePassword = $null
    $keyPassword = $null
